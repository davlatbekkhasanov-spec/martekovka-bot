...
def session_summary(ws: Session) -> str:
    ...
    ish = fmt_clock(work)
    return f"Markirovka: poz {ws.poz}, ish {ish}, norm {NORM_SEC_PER_POZ}s"


def daily_summary(sessions: list[Session]) -> str:
    if not sessions:
        return "Markirovka: poz 0, ish 00:00:00, norm 20s"
    total_poz = sum(int(s.poz or 0) for s in sessions)
    total_ish = sum(int(s.work_sec or 0) for s in sessions)
    return f"Markirovka: poz {total_poz}, ish {fmt_clock(total_ish)}, norm {NORM_SEC_PER_POZ}s"
